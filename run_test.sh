...
nvcc -std=c++17 -O3 -L. -Lpsd_projection/build -lpsd_lib -Ipsd_projection/include -lcublas -lcudart -lcusolver -Wno-deprecated-gpu-targets -o benchmark_test benchmark_full_test.cu || exit 1

# generate the datasets
# julia data/generate.jl --datasets "${datasets[@]}" --instance_sizes "${instance_sizes[@]}" || exit 1

# run the benchmark
./benchmark_test --datasets "${datasets[@]}" --instance_sizes "${instance_sizes[@]}" --restarts "$restarts" --gemmrestarts "$gemmrestarts"